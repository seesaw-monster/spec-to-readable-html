...
4. **Make the output portable**
   - Prefer a self-contained HTML file with embedded CSS.
   - Avoid external dependencies unless the user allows them.
   - If Mermaid or external chart libraries are used, explain the dependency.
...
1. **Inline SVG** for standalone visual diagrams and charts.
2. **Mermaid** for architecture, flowchart, sequence, state, and ER diagrams when source code readability matters.
...
If using Mermaid, include the Mermaid source in the HTML or appendix so it can be edited later.
...
- [ ] The file is standalone or dependencies are documented.
